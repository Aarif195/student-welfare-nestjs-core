    ...
    useEffect(() => {
        ...
        if (user) {
            setFormData({
                ...
                phone: user.phone || '',
                address: (user as any).address || '', // address isn't in your DTO but is in your form
                image: user.image || ''
            });
        }
    }, [response]);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white p-6 rounded-[2.5rem] border border-primary-50 text-center shadow-sm">
                        <div className="relative w-32 h-32 mx-auto mb-4">
                            <img
                                ...
                                alt="Profile"
                                className={`w-full h-full rounded-full object-cover border-4 border-primary-50 shadow-inner ${isUploading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:opacity-80'}`}
                            />
                            <input
                                type="file"
                                ref={fileInputRef}
                            ...
                            {isEditing && (
                                <div
                                    ...
                                    className="absolute bottom-0 right-0 bg-brand p-2 rounded-full text-white shadow-lg cursor-pointer hover:scale-110 transition-all"
                                >
                        ...
                        <div className="inline-flex items-center gap-1 mt-2 px-3 py-1 bg-green-50 text-green-600 rounded-full text-[10px] font-bold uppercase border border-green-100">
                            <ShieldCheck size={12} /> {profile?.role}
                        </div>
                        <p className="text-[10px] text-primary-400 mt-4 flex items-center justify-center gap-1">
                            <Calendar size={12} />
                            Joined {profile?.createdAt ? new Date(profile.createdAt).toLocaleDateString() : 'Loading...'}
                        </p>
                    </div>
                </div>
                ...
                <div className="lg:col-span-2">
                    <form onSubmit={handleUpdate} className="bg-white p-8 rounded-[2.5rem] border border-primary-50 shadow-sm space-y-5">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-bold uppercase text-primary-400 ml-1">Email Address</label>
                            <div className="relative">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-bold uppercase text-primary-400 ml-1">Home Address</label>
                            <div className="relative">
                        ...
                        {isEditing && (
                            <div className="flex gap-3 pt-4">
                                ...
                                <button
                                    ...
                                    className="flex-1 bg-brand text-white py-3.5 rounded-2xl font-bold text-xs uppercase tracking-widest hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-100 cursor-pointer"
                                >
